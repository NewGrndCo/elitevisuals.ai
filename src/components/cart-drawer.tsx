...
export function CartDrawer() {
  const { isOpen, closeCart, lines, checkoutUrl, isLoading, removeItem } = useCart();
  ...
  const currency = lines[0]?.cost.totalAmount.currencyCode ?? "USD";
        ...
        {lines.length > 0 && (
          <footer className="border-t border-[rgba(124,92,252,0.15)] px-6 py-5">
            <div className="mb-4 flex items-center justify-between">
              ...
              <span className="font-display text-lg font-bold">{formatMoney(subtotal, currency)}</span>
            </div>
            <a
              href={checkoutUrl ?? "#"}
              aria-disabled={!checkoutUrl}
              className={`flex w-full items-center justify-center gap-2 rounded-[10px] bg-primary py-3.5 text-center text-base font-bold tracking-[-0.01em] text-primary-foreground shadow-[0_0_28px_rgba(124,92,252,0.35),0_0_60px_rgba(124,92,252,0.18)] transition-all hover:-translate-y-0.5 ${!checkoutUrl ? "pointer-events-none opacity-60" : ""}`}
            >
